...
*/
public class msg_mission_item_test{
...
public CRC generateCRC(byte[] packet){
    ...
    crc.finish_checksum(MAVLINK_MSG_ID_MISSION_ITEM);
    return crc;
}
...
public byte[] generateTestPacket(){
    ...
    payload.put((byte)MAVLINK_MSG_ID_MISSION_ITEM); //msg id
    ...
    payload.putFloat((float)129.0); //x
    payload.putFloat((float)157.0); //y
    payload.putFloat((float)185.0); //z
    payload.putShort((short)18691); //seq
    payload.putShort((short)18795); //command
    payload.put((byte)101); //target_system
    payload.put((byte)168); //target_component
    payload.put((byte)235); //frame
    payload.put((byte)46); //current
    payload.put((byte)113); //autocontinue
...
public void test(){
    ...
    assertArrayEquals("msg_mission_item", processedPacket, packet);
}
}
