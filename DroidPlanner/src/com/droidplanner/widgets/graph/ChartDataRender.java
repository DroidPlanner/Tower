...
public class ChartDataRender {
	...
	protected void drawSeries(Chart chart, Canvas canvas, ChartSeries serie) {
		...
		if (serie.isActive()) {
			...
			int start = (int) ((serie.getFirstIndex()
					- chart.scale.x.getRange() + serie.data.length + chart.scale.x
					.getOffset()) % serie.data.length);
			int pos = 0;
			...
			try {
				for (int i = start; i < start + chart.scale.x.getRange() - 1; i++) {
					...
					y_i = (y_i + chart.scale.y.getRange() - chart.scale.y
							.getOffset())
							/ (2 * chart.scale.y.getRange())
							* chart.height;

					double y_i1 = -serie.data[(i + 1) % serie.data.length];
					y_i1 = (y_i1 + chart.scale.y.getRange() - chart.scale.y
							.getOffset())
							/ (2 * chart.scale.y.getRange())
							* chart.height;
