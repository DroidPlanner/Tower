...
{
	// AL** bug? add  * numIndices
	posteriors = new float[numTrees * numIndices + numIndices];
	positives = new int[numTrees * numIndices + numIndices];
	negatives = new int[numTrees * numIndices + numIndices];
...
void EnsembleClassifier::calcFeatureVector(int windowIdx, int *featureVector)
{
    ...
    for(int i = 0; i < numTrees; i++)
    {
        long tdiff = timerEnd(2); if (tdiff > 1000) { LOGD("EnsembleClassifier taks too long to calcFeatureVector - quit"); return; }
...
float EnsembleClassifier::calcConfidence(int *featureVector)
{
    ...
    for(int i = 0; i < numTrees; i++)
    {
        long tdiff = timerEnd(2); if (tdiff > 1000) { LOGD("EnsembleClassifier taks too long to calcConfidence - quit"); return conf; }
		int featureBase = i * numIndices;
		int featureOffset = featureVector[i];
		int off = featureBase + featureOffset;
        conf += posteriors[off];
    }
...
void EnsembleClassifier::classifyWindow(int windowIdx)
{
	...
	int offsetFeatures = numTrees * windowIdx;
    assert(offsetFeatures >= 0);

    int *featureVector = detectionResult->featureVectors + offsetFeatures;
...
void EnsembleClassifier::updatePosteriors(int *featureVector, int positive, int amount)
{
    ...
    for(int i = 0; i < numTrees; i++)
    {
        long tdiff = timerEnd(2); if (tdiff > 1000) { LOGD("EnsembleClassifier taks too long to updatePosteriors - quit"); return; }
