...
{
	lastNumWindows = numWindows;

    maxFeatureVectors = numWindows * numTrees;

    variances = new float[numWindows];
    posteriors = new float[numWindows];
    featureVectors = new int[maxFeatureVectors]; // AL** HACK
...
void DetectionResult::reset()
{
    ...
    // AL** add clearing feature vectors to avoid crash when after resetting detector results
    memset(featureVectors, 0, maxFeatureVectors*sizeof(int));
