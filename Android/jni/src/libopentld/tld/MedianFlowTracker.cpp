...
void MedianFlowTracker::track(const Mat &prevMat, const Mat &currMat, Rect *prevBB)
{
    if(prevBB != NULL)
    {
        ...
        LOGD("MF fbtrack return from fbtrack");
        ...
        if(!success || x < 0 || y < 0 || w <= 0 || h <= 0 
                    || x + w > currMat.cols 
                    || y + h > currMat.rows 
                    || x != x || y != y || w != w || h != h) //x!=x is check for nan
        {
             LOGD("MF fbtrack FAILED!      success=%d     [%f,%f,%f,%f]****", 
                success, x, y, w, h);
            //Leave it empty
        }
        else
