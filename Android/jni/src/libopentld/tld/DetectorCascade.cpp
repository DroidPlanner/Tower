...
void DetectorCascade::detect(const Mat &img)
{
    ...
    LOGD("DetectorCascade::detect: move classifiers to next iteration");
    //Prepare components
	...
	LOGD("DetectorCascade::detect: check windows count");
	assert(numWindows == detectionResult->lastNumWindows);


    LOGD("DetectorCascade::detect: start");
    timerStart(1);

    for(int i = 0; i < numWindows; i++)
    {
        long timeDiff = timerEnd(1);
        if (timeDiff > 1000)
        {
            LOGD("abort DetectorCascade::detect...  took too long (1secs)");
            return;
        }
        ...
        //LOGD("DetectorCascade::detect: VF");
        ...
        //LOGD("DetectorCascade::detect: EC");
        if(!ensembleClassifier->filter(i))
        {
            continue;
        }

        //LOGD("DetectorCascade::detect: NN");
    ...
    LOGD("DetectorCascade::detect: cluster confidence indices");
    //Cluster
    clustering->clusterConfidentIndices();

    LOGD("DetectorCascade::detect: reached end of windows filtering: found valid data");
