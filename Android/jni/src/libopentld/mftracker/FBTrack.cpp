            ...
            float *scaleshift)
{
    ...
    for(i = 0; i < nlkPoints; i++)
    {
        if((fbLkCleaned[i] <= medFb) & (nccLkCleaned[i] >= medNcc))
        {
            ...
            setTF(nAfterFbUsage, startPoints[i].x, startPoints[i].y, targetPoints[i].x, targetPoints[i].y);
    ...
    LOGD("MT realPoints=%d, medFb=%f, medNcc=%f, nklPoints=%d", 
          nRealPoints, medFb, medNcc, nlkPoints);

    if(medFb > 10)
    { 
        LOGD("MT med points enough");
        return 0;
    }
    else
    { 
        LOGD("MT med points NOT enough");
        return 1;
    }
