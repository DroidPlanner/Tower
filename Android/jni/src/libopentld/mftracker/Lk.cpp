                          ...
                          float *match, int winsize, int method)
{
    ...
    for(i = 0; i < nPts; i++)
    {
        if(status[i] == 1)
        {
            ...
            if (res->imageData)
            { 
            float matchVal = ((float *)(res->imageData))[0];
            match[i] = matchVal;
            }
        }
        else
...
void normCrossCorrelation2(IplImage *imgI, IplImage *imgJ,
    CvPoint2D32f *points0, CvPoint2D32f *points1, int nPts, char *status,
    float *match, int winsize, int method)
{
    IplImage *rec0 = cvCreateImage(cvSize(winsize, winsize), 8, 1);
    ...
    for (i = 0; i < nPts; i++)
    {
        if (status[i] == 1)
        {
            CvPoint2D32f  pts0 = points0[i];
            cvGetRectSubPix(imgI, rec0, pts0);

            CvPoint2D32f  pts1 = points1[i];
            cvGetRectSubPix(imgJ, rec1, points1[i]);
...
void normCrossCorrelation3(IplImage *imgI, IplImage *imgJ,
    CvPoint2D32f *points0, CvPoint2D32f *points1, int nPts, char *status,
    float *match, int winsize, int method)
{
    cv::Mat matI = cv::cvarrToMat(imgI, true);
    cv::Mat matJ = cv::cvarrToMat(imgJ, true);

    int result_cols = winsize; // img.cols - templ.cols + 1;
    int result_rows = winsize; //img.rows - templ.rows + 1;
    ...
    for (size_t i = 0; i < nPts; i++)
    {
        if (status[i] == 1)
        {
            ...
            CvPoint2D32f  pts0 = points0[i];
            CvPoint2D32f  pts1 = points1[i];
            cv::Point2f center0(pts0.x, pts0.y);
            cv::Point2f center1(pts1.x, pts1.y);

            //getRectSubPix(InputArray image, Size patchSize, Point2f center, OutputArray patch, int patchType = -1)
            cv::Mat templPatch0;
            getRectSubPix(matI, cv::Size(winsize, winsize), center0, templPatch0);

            cv::Mat templPatch1;
            getRectSubPix(matJ, cv::Size(winsize, winsize), center1, templPatch1);

            //cv::matchTemplate(templPatch0, templPatch1, result, CV_TM_CCORR);

            cv::Mat templPatchT(cv::Size(10, 10), CV_8UC1);
            cv::Mat templPatchT2(cv::Size(9, 9), CV_8UC1);
            cv::Mat result2(cv::Size(2,2), CV_8UC1);

            int result_cols = 10 - 9 + 1;
            int result_rows = 1 + 1;

            cv::matchTemplate(templPatchT, templPatchT2, result2, CV_TM_CCORR);
            ...
            float ptsJ[], int nPtsJ, int level, float *fb, float *ncc, char *status)
{
    ...
    normCrossCorrelation3(imgI, imgJ, points[0], points[1], nPtsI, status, ncc, 
