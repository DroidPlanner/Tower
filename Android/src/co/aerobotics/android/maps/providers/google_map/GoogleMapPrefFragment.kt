 ...
 */
class GoogleMapPrefFragment : co.aerobotics.android.maps.providers.MapProviderPreferences(), co.aerobotics.android.dialogs.EditInputDialog.Listener {
    ...
    companion object PrefManager {
        ...
        const val DEFAULT_ARCGIS_MAP_TYPE = co.aerobotics.android.R.string.label_nat_geo_world_map
    ...
    private val accessTokenDialog = co.aerobotics.android.dialogs.EditInputDialog.newInstance(MAPBOX_ACCESS_TOKEN_DIALOG_TAG, "Enter mapbox access token", "mapbox access token", false)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        addPreferencesFromResource(co.aerobotics.android.R.xml.preferences_google_maps)
        setupPreferences()
    }
    ...
    private fun isMapboxIdSet() = !TextUtils.isEmpty(getMapboxId(context))

    private fun isMapboxAccessTokenSet() = !TextUtils.isEmpty(getMapboxAccessToken(context))
        ...
        when (dialogTag) {
            MAPBOX_ID_DIALOG_TAG -> {
                if (TextUtils.isEmpty(input)) {
                    Toast.makeText(context, co.aerobotics.android.R.string.label_invalid_mapbox_id, Toast.LENGTH_LONG)
                            .show()
                } else {
            ...
            MAPBOX_ACCESS_TOKEN_DIALOG_TAG -> {
                if(TextUtils.isEmpty(input)){
                    Toast.makeText(context, co.aerobotics.android.R.string.label_invalid_mapbox_access_token,
    ...
    private fun setupTileProvidersPreferences(sharedPref: SharedPreferences) {
        ...
        providerSettingsScreens.put(co.aerobotics.android.maps.providers.google_map.GoogleMapPrefConstants.GOOGLE_TILE_PROVIDER, findPreference(PREF_GOOGLE_TILE_PROVIDER_SETTINGS) as PreferenceCategory)
        providerSettingsScreens.put(co.aerobotics.android.maps.providers.google_map.GoogleMapPrefConstants.MAPBOX_TILE_PROVIDER, findPreference(PREF_MAPBOX_TILE_PROVIDER_SETTINGS) as PreferenceCategory)
        providerSettingsScreens.put(co.aerobotics.android.maps.providers.google_map.GoogleMapPrefConstants.ARC_GIS_TILE_PROVIDER, findPreference(PREF_ARCGIS_TILE_PROVIDER_SETTINGS) as PreferenceCategory)
                    ...
                    if (updatedTileProvider == MAPBOX_TILE_PROVIDER) {
                        ...
                        if (!areMapboxCredentialsSet()) {
                            ...
                            val inputDialog = if (!isMapboxIdSet()) {
                                co.aerobotics.android.dialogs.EditInputDialog.newInstance(MAPBOX_ID_DIALOG_TAG, "Enter mapbox id", "mapbox id", false)
                            } else {
                                if (!isMapboxAccessTokenSet()) {
                                    accessTokenDialog
                                } else {
    ...
    private fun updateMapboxId(id: String, persist: Boolean){
        ...
        mapboxIdPref?.let {
            val summary = if (TextUtils.isEmpty(id)) {
                ...
                getString(co.aerobotics.android.R.string.pref_hint_mapbox_id)
            } else {
                id
            }
    ...
    private fun updateMapboxAccessToken(token: String, persist: Boolean){
        ...
        if(mapboxTokenPref != null) {
            val summary = if (TextUtils.isEmpty(token)) {
                enableTileProvider(GOOGLE_TILE_PROVIDER, true)
                getString(co.aerobotics.android.R.string.pref_hint_mapbox_access_token)
            }
            else {
                token
            }
        ...
        if(persist)
            setMapboxAccessToken(context, token)
    }
    ...
    private fun toggleTileProviderPrefs(@TileProvider tileProvider: String){
        ...
        when(tileProvider){
            co.aerobotics.android.maps.providers.google_map.GoogleMapPrefConstants.GOOGLE_TILE_PROVIDER -> setupGoogleTileProviderPreferences(sharedPref)
            co.aerobotics.android.maps.providers.google_map.GoogleMapPrefConstants.MAPBOX_TILE_PROVIDER -> setupMapboxTileProviderPreferences(sharedPref)
            co.aerobotics.android.maps.providers.google_map.GoogleMapPrefConstants.ARC_GIS_TILE_PROVIDER -> setupArcGISTileProviderPreferences(sharedPref)
        }
    }

    override fun getMapProvider(): co.aerobotics.android.maps.providers.DPMapProvider? = co.aerobotics.android.maps.providers.DPMapProvider.GOOGLE_MAP
}
