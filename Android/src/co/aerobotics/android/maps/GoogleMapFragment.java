...
import co.aerobotics.android.maps.providers.google_map.DownloadMapboxMapActivity;
import co.aerobotics.android.maps.providers.google_map.GoogleMapPrefConstants;
import co.aerobotics.android.maps.providers.google_map.GoogleMapPrefFragment;
import co.aerobotics.android.maps.providers.google_map.tiles.TileProviderManager;
import co.aerobotics.android.maps.providers.google_map.tiles.arcgis.ArcGISTileProviderManager;
import co.aerobotics.android.maps.providers.google_map.tiles.mapbox.MapboxTileProviderManager;
...
import co.aerobotics.android.maps.providers.google_map.tiles.mapbox.MapboxUtils;
...
public class GoogleMapFragment extends SupportMapFragment implements DPMap,
    ...
    private FlightControllerKey gpsSignalLevelKey = FlightControllerKey.create(FlightControllerKey.GPS_SIGNAL_LEVEL);
    ...
    private final Map<Marker, CameraMarker> cameraMarkerMap = new HashMap<>();
    private final Map<Polyline, PolylineInfo> polylinesMap = new HashMap<>();
    private static final String Tag = "google_map_fragment";
        ...
        public void onLocationResult(LocationResult result) {
            ...
            if (userMarker == null) {
                final MarkerOptions options = new MarkerOptions()
                        ...
                        .visible(true)
                        .anchor(0.5f, 0.5f)
                        .icon(BitmapDescriptorFactory.fromResource(co.aerobotics.android.R.drawable.user_location));
    ...
    private final GoogleApiClientTask mGoToMyLocationTask = new GoogleApiClientTask() {
        ...
        public void doRun() {
            ...
            try {
                ...
                Log.e(Tag, "Location services security error");
            }
    ...
    private List<Polygon> aeroviewPolygons = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup viewGroup, Bundle bundle) {
        ...
        mMixpanel = MixpanelAPI.getInstance(getActivity(), DroidPlannerApp.getInstance().getMixpanelToken());
        return view;
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater){
        ...
        inflater.inflate(co.aerobotics.android.R.menu.menu_google_map, menu);
    }

    @Override
    public boolean onOptionsItemSelected(MenuItem item){
        switch(item.getItemId()){
            case co.aerobotics.android.R.id.menu_download_mapbox_map:
    ...
    public void onPrepareOptionsMenu(Menu menu){
        final MenuItem item = menu.findItem(co.aerobotics.android.R.id.menu_download_mapbox_map);
        if(item != null) {
    ...
    public void clearPolygons(){
        for (Polygon polygon: aeroviewPolygons){
            polygon.remove();
        }
        aeroviewPolygons.clear();
    }
    ...
    public void clearCameraMarkers() {
        for (CameraMarker cameraMarker : cameraMarkerMap.values()){
            cameraMarker.removeProxyMarker();
        }
    ...
    public void addMarker(final MarkerInfo markerInfo) {
        ...
        if (googleMap != null) {
            ...
            if(markerInfo.getClass().isInstance(CameraMarker.class)){
                cameraMarkerMap.put(marker, (CameraMarker) markerInfo);
            }
        }
    }
    ...
    @Override
    public void addPolygon(final PolygonOptions polygonOptions, final String tag) {
        ...
        if (!coords.isEmpty()) {
            ...
            getMapAsync(new OnMapReadyCallback() {
                ...
                public void onMapReady(GoogleMap googleMap) {
                    ...
                    if (height > 0 && width > 0) {
                        CameraUpdate animation = CameraUpdateFactory.newLatLngBounds(bounds, width, height, 200);
    ...
    if (product != null && product.isConnected()) {
        ...
        DJISDKManager.getInstance().getKeyManager().getValue(gpsSignalLevelKey, new GetCallback() {
            @Override
            public void onSuccess(Object o) {
                if (o instanceof GPSSignalLevel) {
                    GPSSignalLevel gpsSignalLevel = (GPSSignalLevel) o;
                    if (gpsSignalLevel.value() > 2) {
                        getDroneLocation();
                    } else {
                        Toast.makeText(getActivity().getApplicationContext(), "GPS signal weak", Toast.LENGTH_SHORT).show();
                    }
                }
            }
...
/*
        ...
        if (product != null && product.isConnected()) {
            ...
            if (state.getFlightControllerState() != null && state.getGpsSignalLevel().value() >= 2) {
                final float currentZoomLevel = getMap().getCameraPosition().zoom;
                final LatLong droneLocation;
                droneLocation = new LatLong(state.getDroneLatitude(), state.getDroneLongitude());
    ...
    private void zoomToDroneLocation() {
        if (aircraftLongitude != null && aircraftLatitude != null) {
            final float currentZoomLevel = getMap().getCameraPosition().zoom;
            final LatLong droneLocation;
            droneLocation = new LatLong(aircraftLatitude, aircraftLongitude);
            updateCamera(droneLocation, (int) currentZoomLevel);
        }
    }
    ...
    private void setupMapListeners(GoogleMap googleMap) {
        ...
        googleMap.setOnPolygonClickListener(new GoogleMap.OnPolygonClickListener(){
            ...
            public void onPolygonClick(Polygon polygon) {
                ...
                //polygonData.setSelected(true);
                FloatingActionButton genMissionButton = (FloatingActionButton) getActivity().findViewById(R.id.generateMission);
                if (!DroidPlannerApp.getInstance().getSelectedPolygons().contains(polygon.getTag())) {
                    polygon.setFillColor(Color.argb(150,255,255,255));
                    handler.post(polySelectedRunnable((String) polygon.getTag()));
                    genMissionButton.setVisibility(View.VISIBLE);
                    /*DroidPlannerApp.getInstance().addPolygon((int) polygon.getTag());
                    PolygonData polygonData = DroidPlannerApp.getInstance().polygonMap.get((int) polygon.getTag());
                    polygonData.setSelected(true);
                    DroidPlannerApp.getInstance().polygonMap.put((int) polygon.getTag(), polygonData);
                    */
                    //trackBoundarySelected(polygonData);
                    //LocalBroadcastManager.getInstance(getActivity()).sendBroadcast(new Intent(ACTION_POLYGON_CLICKED));

                } else {
                    polygon.setFillColor(Color.TRANSPARENT);
                    DroidPlannerApp.getInstance().removePolygon((String) polygon.getTag());
                    PolygonData polygonData = DroidPlannerApp.getInstance().polygonMap.get((String) polygon.getTag());
                    polygonData.setSelected(false);
                    DroidPlannerApp.getInstance().polygonMap.put((String) polygon.getTag(), polygonData);
                    if (DroidPlannerApp.getInstance().selectedPolygons.isEmpty()){
                        genMissionButton.setVisibility(View.INVISIBLE);
                    }
                    trackBoundaryDeselected(polygonData);
                }
            }
        });
    ...
    private Runnable polySelectedRunnable(final String tag){
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                DroidPlannerApp.getInstance().addPolygon(tag);
                PolygonData polygonData = DroidPlannerApp.getInstance().polygonMap.get(tag);
                polygonData.setSelected(true);
                DroidPlannerApp.getInstance().polygonMap.put(tag, polygonData);
                trackBoundarySelected(polygonData);
            }
        };
        return runnable;
    ...
    private void trackBoundarySelected(PolygonData polygonData){
        try {
            final JSONObject properties = new JSONObject();
            properties.put("Boundary ID", String.valueOf(polygonData.getId()));
            properties.put("Boundary Name", polygonData.getName());
            mMixpanel.track("FPA: BoundarySelected", properties);
            mMixpanel.flush();
    ...
    private void trackBoundaryDeselected(PolygonData polygonData){
        try {
            final JSONObject properties = new JSONObject();
            properties.put("Boundary ID", String.valueOf(polygonData.getId()));
            properties.put("Boundary Name", polygonData.getName());
            mMixpanel.track("FPA: BoundaryDeselected", properties);
        } catch(final JSONException e) {
            throw new RuntimeException("Could not encode JSON");
        }
    }
