...
public class BasicElevationModelBulkDownloader extends BulkRetrievalThread
{
    ...
    protected Sector[] computeRandomRegions(Sector sector, int div, int numRegions)
    {
        ...
        final double dLat = sector.getDeltaLatDegrees() / div;
        final double dLon = sector.getDeltaLonDegrees() / div;
        ...
        while (regions.size() < numRegions)
        {
            ...
            double maxLat = (row+1 < div) ? sector.minLatitude.degrees + dLat * row + dLat
                    : sector.maxLatitude.degrees;

            double maxLon = (col+1 < div) ? sector.minLongitude.degrees + dLon * col + dLon
                    : sector.maxLongitude.degrees;

            Sector s = Sector.fromDegrees(
                sector.minLatitude.degrees + dLat * row, maxLat,
                sector.minLongitude.degrees + dLon * col, maxLon );
