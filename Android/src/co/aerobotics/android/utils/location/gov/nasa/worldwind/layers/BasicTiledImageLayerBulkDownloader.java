...
import co.aerobotics.android.utils.location.gov.nasa.worldwind.render.GpuTextureTile;
...
public class BasicTiledImageLayerBulkDownloader extends BulkRetrievalThread
{
    ...
    protected synchronized void submitMissingTilesRequests() throws InterruptedException
    {
        ...
        while (this.missingTiles.size() > i && rs.isAvailable())
        {
            ...
            GpuTextureTile tile = this.missingTiles.get(i);
    ...
    protected void callRetrievalListeners(Retriever retriever, GpuTextureTile tile)
    {
    ...
    protected synchronized void removeRetrievedTile(GpuTextureTile tile)
    {
    ...
    protected synchronized void removeAbsentTile(GpuTextureTile tile)
    {
        this.missingTiles.remove(tile);
    ...
    protected Sector[] computeRandomRegions(Sector sector, int div, int numRegions)
    {
        ...
        while (regions.size() < numRegions)
        {
            ...
            Sector s = Sector.fromDegrees(
                ...
                sector.minLatitude.degrees + dLat * row + dLat,
                sector.minLongitude.degrees + dLon * col,
                sector.minLongitude.degrees + dLon * col + dLon);
    ...
    protected List<GpuTextureTile> getMissingTilesInSector(Sector sector, int levelNumber)
        throws InterruptedException
    {
        List<GpuTextureTile> tiles = new ArrayList<GpuTextureTile>();

        GpuTextureTile[][] tileArray = this.layer.getTilesInSector(sector, levelNumber);
        for (GpuTextureTile[] row : tileArray)
        {
    ...
    protected boolean isTileLocalOrAbsent(GpuTextureTile tile)
    {
        if (this.layer.getLevels().isResourceAbsent(tile))
