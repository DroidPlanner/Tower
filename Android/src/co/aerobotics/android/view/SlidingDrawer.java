...
public class SlidingDrawer extends ViewGroup {
    ...
    public SlidingDrawer(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs, co.aerobotics.android.R.styleable.SlidingDrawer, defStyleAttr, defStyleRes);

        int orientation = a.getInt(co.aerobotics.android.R.styleable.SlidingDrawer_android_orientation, ORIENTATION_VERTICAL);
        mVertical = orientation == ORIENTATION_VERTICAL;
        mBottomOffset = (int) a.getDimension(co.aerobotics.android.R.styleable.SlidingDrawer_bottomOffset, 0.0f);
        mTopOffset = (int) a.getDimension(co.aerobotics.android.R.styleable.SlidingDrawer_topOffset, 0.0f);
        mAllowSingleTap = a.getBoolean(co.aerobotics.android.R.styleable.SlidingDrawer_allowSingleTap, true);
        mAnimateOnClick = a.getBoolean(co.aerobotics.android.R.styleable.SlidingDrawer_animateOnClick, true);

        int handleId = a.getResourceId(co.aerobotics.android.R.styleable.SlidingDrawer_handle, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(co.aerobotics.android.R.styleable.SlidingDrawer_content, 0);
        if (contentId == 0) {
