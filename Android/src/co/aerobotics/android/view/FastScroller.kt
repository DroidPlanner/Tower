...
class FastScroller(context: Context, attrs: AttributeSet, defStyleAttr: Int) : LinearLayout(context, attrs, defStyleAttr){
    ...
    init {
        ...
        inflater.inflate(if(orientation == HORIZONTAL) co.aerobotics.android.R.layout.horizontal_fastscroller else co.aerobotics.android.R.layout.vertical_fastscroller, this)

        bubble = findViewById(co.aerobotics.android.R.id.fastscroller_bubble)
        handle = findViewById(co.aerobotics.android.R.id.fastscroller_handle)
    }
    ...
    private fun setPosition(x: Float, y: Float){
        if(orientation == HORIZONTAL) {
            ...
            bubble.x = getValueInRange(0, widthRef - bubbleWidth, ((widthRef - bubbleWidth) * position).toInt()).toFloat()
            val handleWidth = handle.width
            handle.x = getValueInRange(0, widthRef - handleWidth, ((widthRef - handleWidth) * position).toInt()).toFloat()
        }
        else {
