package co.aerobotics.android.tlog.adapters
...
import co.aerobotics.android.tlog.viewers.TLogPositionViewer
import co.aerobotics.android.view.adapterViews.AbstractRecyclerViewFooterAdapter
...
class TLogPositionEventAdapter(context : Context, recyclerView: RecyclerView) :
    ...
    private val lengthUnitProvider : co.aerobotics.android.utils.unit.providers.length.LengthUnitProvider

    init {
        val res = context.resources
        lessAltitudeIcon = res.getDrawable(co.aerobotics.android.R.drawable.ic_file_download_black_24dp)
        sameAltitudeIcon = res.getDrawable(co.aerobotics.android.R.drawable.ic_remove_black_24dp)
        moreAltitudeIcon = res.getDrawable(co.aerobotics.android.R.drawable.ic_file_upload_grey_700_18dp)

        lengthUnitProvider = co.aerobotics.android.utils.unit.UnitManager.getUnitSystem(context).lengthUnitProvider
    }
    ...
    override fun onCreateBasicItemViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val container = LayoutInflater.from(parent.context).inflate(co.aerobotics.android.R.layout.list_item_tlog_position_event, parent, false)
        val thumbnail = container.findViewById(co.aerobotics.android.R.id.event_thumbnail)
        val timestamp = container.findViewById(co.aerobotics.android.R.id.event_timestamp) as TextView
        val altitude = container.findViewById(co.aerobotics.android.R.id.event_altitude) as TextView
    ...
    override fun onCreateFooterViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val container = LayoutInflater.from(parent.context).inflate(co.aerobotics.android.R.layout.list_item_tlog_position_event_loading, parent, false)
        val progressBar = container.findViewById(co.aerobotics.android.R.id.progressBar) as ProgressBar
