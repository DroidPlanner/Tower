package co.aerobotics.android.tlog
...
import co.aerobotics.android.droneshare.data.SessionContract.SessionData
import co.aerobotics.android.tlog.adapters.TLogDataAdapter
import co.aerobotics.android.tlog.adapters.TLogViewerAdapter
 ...
 */ class TLogActivity : co.aerobotics.android.activities.DrawerNavigationUI(), TLogDataAdapter.Listener, TLogDataProvider {
    ...
    private val loadingProgress by lazy {
        findViewById(co.aerobotics.android.R.id.progress_bar_container)
    }

    private val sessionTitleView by lazy {
        findViewById(co.aerobotics.android.R.id.tlog_session_title) as TextView?
    }

    override fun getNavigationDrawerMenuItemId() = 0

    override fun getToolbarId() = co.aerobotics.android.R.id.actionbar_toolbar

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(co.aerobotics.android.R.layout.activity_tlog)

        val viewPager = findViewById(co.aerobotics.android.R.id.container) as ViewPager?
        viewPager?.adapter = TLogViewerAdapter(supportFragmentManager)

        val tabLayout = findViewById(co.aerobotics.android.R.id.tabs) as TabLayout?
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        menuInflater.inflate(co.aerobotics.android.R.menu.menu_locator, menu)
        return true
    }

    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            co.aerobotics.android.R.id.menu_open_tlog_file -> {
            ...
            co.aerobotics.android.R.id.menu_rename_tlog_session -> {
                if (currentSessionData != null) {
                    val renameDialog = co.aerobotics.android.dialogs.SupportEditInputDialog.newInstance(TLogDataAdapter.RENAME_SESSION_TAG,
                            "Enter session label", currentSessionData!!.label, true,
                            object : co.aerobotics.android.dialogs.SupportEditInputDialog.Listener{
                                override fun onOk(dialogTag: String?, input: CharSequence?) {
                                    if (TextUtils.isEmpty(input)) {
                                        Toast.makeText(applicationContext, co.aerobotics.android.R.string.warning_invalid_session_label_entry, Toast.LENGTH_LONG).show()
            ...
            co.aerobotics.android.R.id.menu_delete_tlog_session -> {
                if (currentSessionData != null) {
                    val confirmDialog = co.aerobotics.android.dialogs.OkDialog.newInstance(applicationContext, "Delete?",
                            "Delete session ${currentSessionData!!.label}?",
                            object : co.aerobotics.android.dialogs.OkDialog.Listener {
                                override fun onOk() {
    ...
    private fun onTLogSelected(tlogSession: co.aerobotics.android.droneshare.data.SessionContract.SessionData, force: Boolean) {
    ...
    private fun notifyTLogSelected(tlogSession: co.aerobotics.android.droneshare.data.SessionContract.SessionData) {
