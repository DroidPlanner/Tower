...
import co.aerobotics.android.tlog.adapters.TLogDataAdapter.Listener
...
class TLogDataPicker : DialogFragment(){
    ...
    private val noTLogMessageView by lazy {
        view?.findViewById(co.aerobotics.android.R.id.no_tlogs_message)
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?) : View? {
        return inflater.inflate(co.aerobotics.android.R.layout.fragment_tlog_data_picker, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?){
        ...
        val tlogsView = view.findViewById(co.aerobotics.android.R.id.tlogs_selector) as RecyclerView?
        tlogsView?.setHasFixedSize(true)

        // Use a linear layout manager
        val layoutMgr = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
        tlogsView?.layoutManager = layoutMgr

        val adapter = TLogDataAdapter(activity.application as co.aerobotics.android.DroidPlannerApp, childFragmentManager, currentSessionId)
