...
class MiniWidgetGeoInfo : TowerWidget() {
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater?.inflate(co.aerobotics.android.R.layout.fragment_mini_widget_geo_info, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?){
        super.onViewCreated(view, savedInstanceState)

        latitude = view.findViewById(co.aerobotics.android.R.id.latitude_telem) as TextView?
        longitude = view.findViewById(co.aerobotics.android.R.id.longitude_telem) as TextView?

        val clipboardMgr = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager

        val container = view.findViewById(co.aerobotics.android.R.id.mini_widget_geo_info_layout)
    ...
    private fun onPositionUpdate() {
        ...
        if (droneGps.isValid) {
            ...
            latitude?.text = getString(co.aerobotics.android.R.string.latitude_telem, Location.convert(latitudeValue, Location.FORMAT_DEGREES).toString())
            longitude?.text = getString(co.aerobotics.android.R.string.longitude_telem, Location.convert(longitudeValue, Location.FORMAT_DEGREES).toString())
