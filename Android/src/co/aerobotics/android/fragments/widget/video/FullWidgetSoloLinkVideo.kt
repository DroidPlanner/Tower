 ...
 */
class FullWidgetSoloLinkVideo : BaseVideoWidget() {
    ...
    private var fpvLoader: co.aerobotics.android.dialogs.LoadingDialog? = null

    private var surfaceRef: Surface? = null

    private val textureView by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(co.aerobotics.android.R.id.sololink_video_view) as TextureView?
    }

    private val videoStatus by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(co.aerobotics.android.R.id.sololink_video_status) as TextView?
    }
    ...
    private val takePhotoButton by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(co.aerobotics.android.R.id.sololink_take_picture_button)
    }

    private val recordVideo by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(co.aerobotics.android.R.id.sololink_record_video_button)
    }

    private val fpvVideo by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(co.aerobotics.android.R.id.sololink_vr_video_button)
    }

    private val touchCircleImage by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(co.aerobotics.android.R.id.sololink_gimbal_joystick)
    }
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater?.inflate(co.aerobotics.android.R.layout.fragment_widget_sololink_video, container, false)
    }
    ...
    private fun launchFpvApp() {
        ...
        if (launchIntent == null) {
            ...
            if(fpvLoader == null) {
                ...
                fpvLoader = co.aerobotics.android.dialogs.LoadingDialog.newInstance("Starting FPV...", object : co.aerobotics.android.dialogs.LoadingDialog.Listener {
            ...
            override fun onSuccess() {
                ...
                val gimbalTracker = object : View.OnTouchListener {
                    ...
                    private fun moveCopter(view: View, event: MotionEvent): Boolean {
                        ...
                        when (event.action) {
                            MotionEvent.ACTION_DOWN -> {
                                ...
                                touchCircleImage?.visibility = View.VISIBLE
                                touchCircleImage?.x = xTouch - centerTouchX
                                touchCircleImage?.y = yTouch - centerTouchY
                                startX = event.x
                                startY = event.y
                                return true
                            }
                            ...
                            MotionEvent.ACTION_UP -> {
                                touchCircleImage?.visibility = View.GONE
                    ...
                    private fun sendYawAndPitch(view: View, event: MotionEvent, rotateTo: Float) {
                        val orientation = GimbalApi.getApi(drone).gimbalOrientation

                        val degreeIntervals = 90f / view.height
                        val pitchDegree = (degreeIntervals * (startY - event.y)).toFloat()
                        val pitchTo = orientation.pitch + pitchDegree

                        Timber.d("Pitch %f roll %f yaw %f", orientation.pitch, orientation.roll, rotateTo)
                        Timber.d("degreeIntervals: %f pitchDegree: %f, pitchTo: %f", degreeIntervals, pitchDegree, pitchTo)

                        GimbalApi.getApi(drone).updateGimbalOrientation(pitchTo, orientation.roll, rotateTo, orientationListener)
                    }
                }
