...
abstract class BaseUVCVideoWidget : TowerWidget(){
    ...
    protected val textureView by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(co.aerobotics.android.R.id.uvc_video_view) as TextureView?
    }

    protected val videoStatus by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(co.aerobotics.android.R.id.uvc_video_status) as TextView?
    }
        ...
        override fun onConnect(device: UsbDevice, ctrlBlock: USBMonitor.UsbControlBlock, createNew: Boolean) {
            ...
            EXECUTER.execute(object : Runnable {
                override fun run() {
                    ...
                    if (DEBUG) Log.i(TAG, "supportedSize:" + mUVCCamera?.supportedSize)
    ...
    protected fun startVideoStreaming(){
        ...
        if (usbDevice != null) {
            ...
            //UVC Device Filter
            val uvcFilter = DeviceFilter.getDeviceFilters(activity, co.aerobotics.android.R.xml.uvc_device_filter)
            val uvcDevices = mUSBMonitor?.getDeviceList(uvcFilter[0])
            if (uvcDevices == null || uvcDevices.isEmpty()) {
                if (DEBUG) Log.v(TAG, getString(co.aerobotics.android.R.string.uvc_device_no_device))
            } else {
                if (uvcDevices.size.compareTo(1) == 0) {
                    usbDevice = uvcDevices.get(0)
                    mUSBMonitor?.requestPermission(usbDevice)
                } else {
                    co.aerobotics.android.dialogs.UVCDialog.showDialog(activity, mUSBMonitor)
                }
            }
        }
    }
