...
class MiniWidgetWeatherInfo : TowerWidget() {
    ...
    private val gapiClientManager by lazy {
        GoogleApiClientManager(context, handler,  arrayOf(LocationServices.API))
    }

    private val windSpeed by lazy {
        view?.findViewById(co.aerobotics.android.R.id.weather_wind_speed) as TextView
    }

    private val precipitationChances by lazy {
        view?.findViewById(co.aerobotics.android.R.id.weather_precipitation) as TextView
    }
    private val temperature by lazy {
        view?.findViewById(co.aerobotics.android.R.id.weather_temperature) as TextView
    }

    private val weatherLocation by lazy {
        view?.findViewById(co.aerobotics.android.R.id.weather_location) as TextView
    }

    private var weatherAsyncTask: co.aerobotics.android.fragments.widget.weather.WeatherFetcher? = null
    ...
    private val fetchWeatherTask = object : GoogleApiClientManager.GoogleApiClientTask() {
        override fun doRun() {
            ...
            if (userLocation != null) {
                ...
                weatherAsyncTask = co.aerobotics.android.fragments.widget.weather.WeatherFetcher(this@MiniWidgetWeatherInfo, userLocation)
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater?.inflate(co.aerobotics.android.R.layout.fragment_mini_widget_weather_info, container, false)
    }
    ...
    private fun getWeatherUrlPath(location: Location): String {
        return getString(co.aerobotics.android.R.string.wunderground_url, WEATHER_API_TOKEN, location.latitude, location.longitude)
    }
    ...
    private fun processWeatherInfo() {
        ...
        try {
            val weatherJSONObject = JSONObject(appPrefs.prefWeatherInfo)
            ...
            if (expiryTime < System.currentTimeMillis()) {
                if(co.aerobotics.android.utils.NetworkUtils.isNetworkAvailable(context)) {
    ...
    fun onJSONRetrieved(serverResponse: JSONObject?) {
        ...
        if (weather != null) {
            ...
            appPrefs.prefWeatherInfo = weather.toString()
    ...
    private fun updateViews(jsonObject: JSONObject) {
        ...
        val unitSystemType = co.aerobotics.android.utils.unit.UnitManager.getUnitSystem(context).type
        ...
        when (unitSystemType) {
            co.aerobotics.android.utils.unit.systems.UnitSystem.IMPERIAL -> {
                ...
                if (!TextUtils.isEmpty(temperatureImperial) && temperatureImperial != NO_WEATHER_DATA) {
                    temperature.text = getString(co.aerobotics.android.R.string.weather_temperature_imperial, temperatureImperial)
                }
                ...
                if (!TextUtils.isEmpty(windSpeedImperial) && windSpeedImperial != NO_WEATHER_DATA && isWindDirectionValid) {
                    windSpeed.text = getString(co.aerobotics.android.R.string.weather_wind_velocity_imperial, windSpeedImperial, windDirection)
                }
                ...
                if (!TextUtils.isEmpty(precipitation) && precipitation != NO_WEATHER_DATA) {
                    precipitationChances.text = getString(co.aerobotics.android.R.string.weather_rain_imperial, precipitation)
                }
            }

            co.aerobotics.android.utils.unit.systems.UnitSystem.METRIC -> {
                ...
                if (!TextUtils.isEmpty(temperatureMetric) && temperatureMetric != NO_WEATHER_DATA) {
                    temperature.text = getString(co.aerobotics.android.R.string.weather_temperature_metric, temperatureMetric)
                }
                ...
                if (!TextUtils.isEmpty(windSpeedMetric) && windSpeedMetric != NO_WEATHER_DATA && isWindDirectionValid) {
                    windSpeed.text = getString(co.aerobotics.android.R.string.weather_wind_velocity_metric, windSpeedMetric, windDirection)
                }
                ...
                if (!TextUtils.isEmpty(precipitationMetric) && precipitationMetric != NO_WEATHER_DATA) {
                    precipitationChances.text = getString(co.aerobotics.android.R.string.weather_rain_metric, precipitationMetric)
                }
            }
        }
    }
    ...
    fun fetchWeatherInformationRequest(location: Location): JSONObject? {
        ...
        try {
            val httpClient = co.aerobotics.android.utils.NetworkUtils.getHttpClient()
