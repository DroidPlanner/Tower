...
public class YesNoWithPrefsDialog extends YesNoDialog {
	...
	public static YesNoWithPrefsDialog newInstance(Context context, String title, String msg,
			String positiveLabel, String negativeLabel, Listener listener, String prefKey) {
	...
	protected AlertDialog.Builder buildDialog(Bundle savedInstanceState) {
		...
		builder.setPositiveButton(arguments.getString(EXTRA_POSITIVE_LABEL),
				...
				}).setNegativeButton(arguments.getString(EXTRA_NEGATIVE_LABEL),
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
	...
	private void savePreferences(final String prefKey, final boolean isPositiveResponse) {
		if (mCheckbox != null) {
			...
			if (dontShow) {
				Toast.makeText(getActivity(), R.string.pref_dialog_selection_reset_desc,
						Toast.LENGTH_LONG).show();
				editor.putString(prefKey, getString(isPositiveResponse ? PREFERENCE_ALWAYS_ID
						: PREFERENCE_NEVER_ID));
			} else {
			...
			HitBuilders.EventBuilder eventBuilder = new HitBuilders.EventBuilder()
					...
					.setLabel(
							"Response: " + (isPositiveResponse ? "Yes" : "No")
