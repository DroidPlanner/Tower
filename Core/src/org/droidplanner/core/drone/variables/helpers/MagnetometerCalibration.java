...
public class MagnetometerCalibration implements OnDroneListener {
	...
	public MagnetometerCalibration(Drone drone, OnMagCalibrationListener listener,
			DroneInterfaces.Handler handler) {
		this.drone = drone;
	...
	public void start(List<? extends ThreeSpacePoint> newPoints) {
		...
		MavLinkStreamRates
				.setupStreamRates(drone.getMavClient(), 0, 0, 0, 0, 0, 0, REFRESH_RATE, 0);
	...
	void addpoint(Drone drone) {
		...
		ThreeSpacePoint point = new ThreeSpacePoint(mag.getX() - offsets[0], mag.getY()
				- offsets[1], mag.getZ() - offsets[2]);
		points.add(point);
	}
	...
	void fit() {
		...
		if (!fitComplete && ellipsoidFit.getFitness() > ELLIPSOID_FITNESS_MIN
				&& points.size() > MIN_POINTS_COUNT) {
			fitComplete = true;
			if (listener != null) {
	...
	public void sendOffsets() throws Exception {
		...
		drone.getParameters().sendParameter(offsetX); // TODO should probably do
														// a check after sending
														// the parameters
